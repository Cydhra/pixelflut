...
fn main() {
    let matches = App::new("pixelflut")
        ...
        .author("cydhra")
        .about(
            "one network to rule them all, one network to find them, one network to bring them \
        ...
        .arg(
            Arg::with_name("instances")
                ...
                .takes_value(true)
                .help(
                    "how many instances of the image to draw (each with the specified number of \
    ...
    let thread_count = matches
        ...
        .map(|v| v.parse().unwrap())
        .unwrap_or(16u32);
    let instances_count = matches
        .value_of("instances")
        .map(|v| v.parse().unwrap())
        .unwrap_or(1);
    let column_width = width / thread_count;
    println!(
        "will divide image into {} columns of width {} and will render each column in {} \
         threads",
        thread_count, column_width, instances_count
    );
